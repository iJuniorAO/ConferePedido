...
st.set_page_config(page_title="Central Mumix - Jira", layout="wide", page_icon=":material/local_post_office:")
    ...
    try:
        ...
        for issue in issues:
            ...
            dados.append({
                ...
                "descricao": issue.fields.description if issue.fields.description else "Sem descrição informada.",
...
with st.sidebar:
    ...
    filtro_projeto = st.text_input("Código do Projeto (ex: MTED)", help="Deixe em branco para ver todos os projetos")

    # Filtro de Status
    filtro_status = st.selectbox(
        "Status da Tarefa",
        ["Todas", "To Do", "In Progress", "Done", "Backlog"]
    )

    # Configuração de Paginação na lateral
    st.markdown("---")
    st.markdown("# :material/Docs: Paginação")
    itens_por_pagina = st.slider("Itens por página", min_value=5, max_value=50, value=20)
...
st.markdown(f"Link das Lojas: [:material/Content_Paste: Formulário Trocas e Devoluções]({link_formulario})")
st.divider()
...
if not df.empty:
    ...
    aba_board, aba_view, aba_coleta = st.tabs(["📊 Quadro (Board)","📋 Painel de Tarefas (detalhado)", "📦 Ordem Coleta"])
        ...
        for _, row in df.iterrows():
            ...
            with col_alvo:
                with st.container(border=True):
                    ...
                    st.markdown(f"**{row['cod_produto']}** | {row['qtd_produto']}x {row['desc_produto']}")
                    st.markdown(f"[🔗 Abrir no Jira]({st.secrets['jira']['url']}/browse/{row['key']})")
    ...
    with aba_view:
        ...
        with st.spinner("Buscando tarefas no Jira..."):
            try:
                ...
                issues = jira.search_issues(jql_final, startAt=inicio_registro, maxResults=itens_por_pagina)
                total_issues = issues.total # Total de tarefas que correspondem ao filtro no Jira
                ...
                if issues:
                    ...
                    st.write(f"Exibindo registros de {inicio_registro + 1} a {fim_registro}")
                    ...
                    for issue in issues:
                        ...
                        with st.expander(titulo_card):
                            ...
                            with col2:
                                ...
                                st.markdown(f"[🔗 Abrir diretamente no Jira]({st.secrets['jira']['url']}/browse/{issue.key})")
                            
                            st.markdown("---")
                            st.write("**Descrição:**")
                            st.write(issue.fields.description if issue.fields.description else "*Sem descrição informada.*")
                else:
    ...
    with aba_coleta:
        ...
        if not df_coleta.empty:
            ...
            for loja in lojas_coleta:
                ...
                df_loja_coleta = df_loja_coleta[["cod_produto", "qtd_produto", "desc_produto", "key","Coletado"]]
                # Renomeia colunas para melhor visualização
                df_loja_coleta.columns = ["Cód. Produto", "Qtd", "Descrição", "Ticket", "Coletado"]
            ...
            st.success("Nenhuma tarefa no status 'Envio Produto' para gerar ordens de coleta.")
