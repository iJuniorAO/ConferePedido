...
COLUNAS_VALIDADE = ["CodProduto", "Descricao", "Tipo", "Estoq", "qtContada", "avaria", "validade", "Prazo", "Status", "obs"]
COLUNAS_PENDENTE = ["CodProduto", "Descricao", "Tipo", "Estoq", "qtContada", "SaldoPendencia"]
COLUNAS_ORDEM_TOTAL = ["CodProduto", "Descricao", "Tipo", "Estoq", "qtContada", "avaria", "validade", "Prazo", "Status", "obs", "SaldoPendencia"]
...
def inserir_validade(codigo: str, validade: datetime.date, qt_contada: float, avaria: bool = False, obs: str = ""):
    ...
    try:
        ...
        if response.data:
            ...
            st.cache_data.clear()
            confirmar_e_atualizar(f"Validade do código {codigo} inserida com sucesso!", True)
        else:
            st.error("Erro ao inserir dados no banco.")
    except Exception as e:
        st.error(f"Erro ao conectar ou atualizar o Banco de Dados: {e}")

def atualizar_qt(codigo: str, validade: datetime.date, nova_qtd: int, avaria: bool = False):
    ...
    try:
        ...
        if response.data:
            ...
            st.cache_data.clear()
            confirmar_e_atualizar(f"Quantidade do produto {codigo} atualizada para {nova_qtd}!", True)
        else:
...
def puxar_tabela_validade() -> pd.DataFrame:
    ...
    try:
        ...
        if not tratar_erros_supabase(e):
            st.stop()
        return pd.DataFrame(columns=["CodProduto", "validade", "qtContada", "avaria", "obs"])
...
def reconciliar_estoque_e_validades(df_db, df_txt):
    ...
    with st.spinner("Validando estoque atual...", show_time=True):
        ...
        estoque_atual = df_txt[df_txt["Estoq"]>0].set_index("CodProduto")["Estoq"].to_dict()
        ...
        for cod_prod in produtos_no_bd:
            ...
            if qt_estoque <= 0:
                if not df_prod.empty:
                    supabase.table("validades").delete().eq("CodProduto", cod_prod_str).eq("avaria",False).execute()
                    Mudou_algo = True
                continue
            ...
            if soma_contada > qt_estoque:
                ...
                if len(df_prod) == 1:
                    ...
                    validade_alvo = str(df_prod_ord.iloc[0]["validade"]) if pd.notna(df_prod_ord.iloc[0]["validade"]) else "null"
                    
                    query = supabase.table("validades").update({"qtContada": qt_estoque}).eq("CodProduto", cod_prod_str).eq("avaria",False)
                ...
                # Se houver linhas múltiplas/duplicadas
                    ...
                    for _, linha in df_prod_ord.iterrows():
                        ...
                        if qtd_linha <= excesso:
                            ...
                            "Cod: ",cod_prod," | QtdContada: ",soma_contada," | qt_estoque: ",qt_estoque, "Deletando..."
                            excesso -= qtd_linha
                            query = supabase.table("validades").delete().eq("CodProduto", cod_prod_str).eq("qtContada", qtd_linha).eq("avaria",False)
                            query = query.is_("validade", "null") if pd.isna(linha["validade"]) else query.eq("validade", validade_alvo)
                            query.execute()
                        else:
                            # Excesso diminui apenas uma parte da linha e encerra
                            nova_qtd = qtd_linha - excesso
                            excesso = 0
                            query = supabase.table("validades").update({"qtContada": nova_qtd}).eq("CodProduto", cod_prod_str).eq("qtContada", qtd_linha).eq("avaria",False)
                            query = query.is_("validade", "null") if pd.isna(linha["validade"]) else query.eq("validade", validade_alvo)
                            query.execute()
...
df_validade["SaldoPendencia"] = df_validade["Estoq"] - df_validade.groupby("CodProduto")["qtContada"].transform("sum")
...
df_emBranco = df_validade[(df_validade["Status"] == "2.Em Branco") | (df_validade["SaldoPendencia"] > 0)]
...
with aba_criticos:
    ...
    with aba_critico_geral:
        if not df_critico.empty:
            st.dataframe(df_critico[COLUNAS_VALIDADE].sort_values(by=["Status","Prazo"]), width="stretch")
        else:
    ...
    with aba_critico_seco:
        if not df_critico_seco.empty:
            ...
            st.dataframe(df_critico_seco[COLUNAS_VALIDADE].sort_values(by=["Status","Prazo"]), width="stretch")
        else:
            st.info("Nenhuma item crítico no momento.")
    with aba_critico_cong:
        if not df_critico_cong.empty:
            st.markdown(f"##### :blue[{len(df_critico_cong)}] Itens CONG/REFR")
            st.dataframe(df_critico_cong[COLUNAS_VALIDADE].sort_values(by=["Status","Prazo"]), width="stretch")
        else:
            st.info("Nenhuma item crítico no momento.")
    

with aba_pendentes:
    st.markdown(f"### :blue[{len(df_emBranco)}] Produtos Aguardando Bipagem / Lançamento")
    ...
    with aba_pendente_geral:
        if not df_emBranco.empty:
            ...
            st.dataframe(df_emBranco[COLUNAS_PENDENTE].sort_values(by="SaldoPendencia", ascending=False), width="stretch")
        else:
            st.success(":material/Person_Celebrate: Excelente! Todos os produtos do estoque possuem suas validades correspondentes.")
    with aba_pendente_seco:
        if not df_pendente_seco.empty:
            st.markdown(f"##### :blue[{len(df_pendente_seco)}] Itens SECO")
            st.dataframe(df_pendente_seco[COLUNAS_PENDENTE].sort_values(by="SaldoPendencia", ascending=False), width="stretch")
        else:
            st.success(":material/Person_Celebrate: Excelente! Todos os produtos do estoque possuem suas validades correspondentes.")
    with aba_pendente_cong:
        if not df_pendente_cong.empty:
            st.markdown(f"##### :blue[{len(df_pendente_cong)}] Itens CONG/REFR")
            st.dataframe(df_pendente_cong[COLUNAS_PENDENTE].sort_values(by="SaldoPendencia", ascending=False), width="stretch")
        else:
            st.success(":material/Person_Celebrate: Excelente! Todos os produtos do estoque possuem suas validades correspondentes.")
...
with aba_geral:
    ...
    st.dataframe(df_validade[COLUNAS_ORDEM_TOTAL].sort_values(by="Status"), width="stretch")
...
with st.form(key="form_nova_validade", clear_on_submit=True):
    ...
    with col1:
        codigo_input = st.text_input("Código do Produto", placeholder="Bipe ou digite o código")
    with col2:
        validate_input = st.date_input("Data de Validade", value=datetime.today(), format="DD/MM/YYYY")
    with col3:
    ...
    with col5:
        obs_input = st.text_input("Observações", placeholder="Ex: Lote promocional / Avaria / Aguardando Fornecedor...")
    ...
    if not codigo_input.strip():
        ...
        if estoque_prod.empty:
            ...
            if (ja_contado + qt_input) > total_estoque_sistema:
                st.error(f"Erro: A quantidade inserida ({qt_input}) somada ao que já foi contado ({ja_contado}) ultrapassa o estoque do sistema ({total_estoque_sistema}).")
            else:
                inserir_validade(
                    codigo=codigo_input,
...
with st.form(key="form_atualizar_qt", clear_on_submit=True):
    ...
    with col_atualiza1:
        codigo_atualiza_input = st.text_input("Codigo", placeholder="Bipe ou digite o codigo")
    with col_atualiza2:
        validade_atualiza_input = st.date_input("Validade", value=datetime.today(), format="DD/MM/YYYY")
    with col_atualiza3:
        nova_qtd_input = st.number_input("Nova quantidade", min_value=0, step=1, value=0)
...
def deletar_validade(codigo: str, validade: datetime.date, avaria: bool = False):
    ...
    try:
        ...
        if response.data:
            ...
            st.cache_data.clear()
            confirmar_e_atualizar(f"Registro do produto {codigo} deletado com sucesso!", True)
            # st.rerun()
        else:
...
with st.form(key="form_deletar_validade", clear_on_submit=True):
    ...
    with col_del1:
        codigo_del_input = st.text_input("Código para Deletar", placeholder="Bipe ou digite o código")
    with col_del2:
        validade_del_input = st.date_input("Data do Registro", value=datetime.today(), format="DD/MM/YYYY", key="del_date")
