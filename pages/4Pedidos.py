...
with col1:
    data_inicial = st.date_input(
        "Selecione a data inicial", value=None, format="DD/MM/YYYY", key="1"
    )
with col2:
    data_final = st.date_input(
        "Selecione a data final", value=None, format="DD/MM/YYYY", key="2"
    )
        ...
        if data_inicial <= data_final:
            ...
            if not pedidos_filtrados.empty:
                st.dataframe(
                    ...
                    column_config={
                        "data_pedido": st.column_config.DateColumn(
                            "data_pedido", format="DD/MM/YYYY"
                        )
                    },
                    hide_index=True,
                )
