...
def calculos(
    ...
    def solicita_guia(df):
        ...
        guia_ST = st.data_editor(
            guia_ST, width="stretch", hide_index=True, disabled=["Descrição"]
        )
        return guia_ST
    ...
    if not linhas_com_guia.empty:
        ...
        if st.toggle("Já possuo retorno da contabilidade"):
            ...
            st.caption(
                "Para seguir sem calcular a guia, insira Valor da Guia qualquer número abaixo de 0,01 | :blue[Ex.: R$ 0,00000001]"
            )
...
with c1:
    uploaded_file = st.file_uploader(
        "COMPRA - Arraste o XML da nota fiscal aqui", type="xml"
    )
with c2:
    uploaded_file_2 = st.file_uploader(
        "BONIFICAÇÃO - Arraste o XML da nota fiscal aqui", type="xml", disabled=True
    )
...
if not pendencia_calculo:
    if uploaded_file_2:
        if uploaded_file_2:
            ...
            if desconsidera_bonificacao == 100:
                st.error(
                    ":material/Close: Não é possível calcular bonificação ignorando 100% da bonificação"
                )
        ...
        resposta_calc = calculos(
            resposta_xml["df"],
            resposta_xml["total_nf"],
            resposta_xml["outras_despesas"],
            resposta_xml["emitente"],
            fator_conversao,
            unidade_compra,
            ignora_impostos,
            None,
    ...
    diferenca_nf_total_calculado = (
        calc_vl_total_calculado - resposta_xml["soma_produtos"]
    )
    valida_calculo()
    ...
    with col1:
        st.metric(
            ...
            f"R$ {resposta_xml["total_nf"]:,.2f}".replace(",", "x")
            .replace(".", ",")
            .replace("x", "."),
        )
        st.space()
        st.metric(
            ...
            f"R$ {resposta_xml["soma_produtos"]:,.2f}".replace(",", "x")
            .replace(".", ",")
            .replace("x", "."),
        )
    with col2:
        st.metric(
            ...
            f"R$ {calc_vl_total_calculado:,.2f}".replace(",", "x")
            .replace(".", ",")
            .replace("x", "."),
        )
        st.space()
        if diferenca_nf_total_calculado > 0:
            st.metric(
                ":red[**Imposto Somado**]",
                f"R$ {abs(diferenca_nf_total_calculado):,.2f}",
            )
        elif calc_vl_total_calculado == resposta_xml["total_nf"]:
            st.metric(
                ":green[**Sem Guia**]", f"R$ {abs(diferenca_nf_total_calculado):,.2f}"
            )
        else:
            st.metric(
                ":green[**Desconto em Boleto**]",
                f"R$ {abs(calc_vl_total_calculado-resposta_xml["total_nf"]):,.2f}".replace(
                    ",", "x"
                )
                .replace(".", ",")
                .replace("x", "."),
    ...
    st.markdown(
        f"## :material/Payments: {meio_pagamento.get(meio_pgto,f"Cod.: {meio_pgto} - :red[Não Encontrado]")}"
    )
    if not resposta_xml["Boletos"]:
        st.markdown(
            f"## :red[:material/Money_Off: Sem Detalhes de Vencimento do Boleto]"
        )
        if calc_vl_total_calculado == valor_pgto:
            st.markdown(
                f"### Valor Total :green[R$ {valor_pgto:,.2f}]".replace(".", "x")
                .replace(",", ".")
                .replace("x", ",")
            )
        else:
            st.markdown(
                f"### Valor Total :red[R$ {valor_pgto:,.2f}]".replace(".", "x")
                .replace(",", ".")
                .replace("x", ",")
            )
        ...
        vl_total_boleto = round(
            sum(float(boleto["vDup"]) for boleto in resposta_xml["Boletos"]), 2
        )
        st.markdown(f"### Boletos Emitidos: {qt_Boleto}")

        if (calc_vl_total_calculado == vl_total_boleto) & (
            vl_total_boleto == resposta_xml["total_nf"]
        ):
            st.markdown(
                f"### Valor Total :green[R$ {vl_total_boleto:,.2f}]".replace(".", "x")
                .replace(",", ".")
                .replace("x", ",")
            )
            st.caption(
                "Boleto :green[**IGUAL**] ao valor total da NFe e o valor calculado"
            )
        elif (calc_vl_total_calculado > vl_total_boleto) & (
            vl_total_boleto == resposta_xml["total_nf"]
        ):
            st.markdown(
                f"### Valor Total :green[R$ {vl_total_boleto:,.2f}]".replace(".", "x")
            ...
            st.caption(
                "Valor a pagar :red[**MAIOR**] que valor total da NFe e IGUAL ao valor calculado"
            )
        elif vl_total_boleto == resposta_xml["soma_produtos"]:
            st.markdown(
                f"### Valor Total :blue[R$ {vl_total_boleto:,.2f}]".replace(".", "x")
                .replace(",", ".")
                .replace("x", ",")
            )
        else:
        ...
        for i, boleto in enumerate(resposta_xml["Boletos"]):
            ...
            with cols[i]:
                with st.container(border=True):
                    ...
                    st.markdown(
                        f"Valor: {float(boleto["vDup"]):,.2f}".replace(".", "x")
                        .replace(",", ".")
                        .replace("x", ",")
                    )
    ...
    if df_calc["Valor Guia"].sum() > 0:
        ...
        df_dir = df_calc[
            ["Item", "Descrição", "Qt de Cx", "Un por Cx", "Valor Total", "Valor un"]
        ]
