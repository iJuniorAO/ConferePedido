...
if arquivo_upload:
    ...
    with st.expander(":material/Delete: Desejo Remover títulos"):
        ...
        st.markdown(
            ":material/Close_Small: Selecione os lançamentos que deseja remover"
        )
    ...
    fluxo_caixa = (
        df.groupby(["Data_Caixa", "Tipo"])["Valor"].sum().unstack(fill_value=0)
    )
    ...
    st.dataframe(
        fluxo_dia.style.format("R$ {:,.2f}").applymap(negativo_vermelho), height=720
    )
else:
