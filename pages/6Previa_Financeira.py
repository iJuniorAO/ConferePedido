...
def calcular_data_caixa(row):
    ...
    if row["Tipo"] == "P":
        if wd == 5:
            return dt + timedelta(days=2)  # Sábado para Segunda
        if wd == 6:
            return dt + timedelta(days=1)  # Domingo para Segunda
        return dt

    # Regra Receber (R): D+1 e Regras de Fim de Semana
    if row["Tipo"] == "R":
        if wd == 4:
            return dt + timedelta(days=3)  # Sexta para Segunda
...
st.markdown("# :material/Chart_Data: Previa Financeira")
...
with st.sidebar:
    ...
    ignorar_previa = st.toggle("Ignorar Previa", value=True)
    ...
    if valor_inicial:
        st.caption(
            f"Saldo do Banco R$ {valor_inicial:,.2f}".replace(",", "x")
            .replace(".", ",")
            .replace("x", ".")
        )
...
if arquivo_upload is None:
    st.info("Faça o upload da planilha Excel para processar os dados.")
    st.stop()


# --- PROCESSAMENTO ---
resp = ler_planilha(arquivo_upload, colunas=COLUNAS_PLANILHA)
...
with st.expander(":material/Delete: Desejo Remover títulos"):
    ...
    remover_todas_linhas = st.multiselect(
        ":material/Close: Remover todos títulos: ",
        options=df["Título"].unique(),
        placeholder="Selecione uma vez e remova todos títulos",
    )
...
st.space()
st.markdown("## :material/Bar_Chart: Controle de Fluxo de Caixa")

# Métricas de Resumo
m1, m2, m3 = st.columns(3)
# m1
m1.metric(
    ":green[:material/Place_Item: Total a Receber]",
    f"R$ {fluxo_dia['Receber'].sum():,.2f}",
)
# m2
m2.metric(
    ":red[:material/Move_Item: Total a Pagar]",
    f"R$ {fluxo_dia['Pagar'].sum():,.2f}",
)
# m3 com diferença de cor
if diferenca_saldo >= 0:
    m3.metric(
        ":material/Money_range: Saldo Final Projetado",
        f"R$ {fluxo_dia['Saldo_Dia'].iloc[-1]:,.2f}",
        delta=f"+ R$ {abs(diferenca_saldo):,.2f}",
    )
else:
    m3.metric(
        ":material/Money_range: Saldo Final Projetado",
...
st.dataframe(fluxo_dia.style.format("R$ {:,.2f}").map(negativo_vermelho), height=720)
