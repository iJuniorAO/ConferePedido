...
# --- CONF PAGINA
st.set_page_config(
    page_title="Lista Atacado", layout="wide", initial_sidebar_state="collapsed"
)
...
with col3:
    f_tabela_preco = st.file_uploader(
        ":material/Add: Arquivo 00001produtotabela.txt",
        disabled=desativa_manual,
        type="txt",
    )
...
with c2:
    ...
    ind_qtmin = st.number_input(
        ":material/Numbers: Estoque Mínimo",
        value=None,
...
if (f_produto and f_extra and f_tabela_preco) or desativa_manual:
    with st.status("Processando dados...", expanded=True) as status:
        ...
        df_merge = df_merge[
            ["CodProduto", "CodGrupo", "Descricao", "Estoq", "Fam", "ListaCodCaract"]
        ]
        ...
        df_completo["Fornecedor"] = df_completo["Descricao"].str.extract(
        ...
        df_completo.loc[
            df_completo["ListaCodCaract"].astype(str).str.contains("000002"), "TIPO"
        ] = "PESO"
        ...
        st.write(
            f":material/Delete: Selecione os produtos que deseja remover da lista original: :red[{len(df_completo)} itens]"
        )
        ...
        if habilita_preco_cx:
            ...
            df_removido["CONV"] = np.where(ultimo.str.isdigit(), ultimo, 1).astype(
                float
            )

            df_removido["Venda"] = df_removido["CONV"] * df_removido["Lojas"]

        df_removido["Lojas"] = df_removido["Lojas"].map(
            lambda x: f"R$ {x:,.2f}".replace(",", "X")
            .replace(".", ",")
            .replace("X", ".")
        )
        df_removido["Atacado"] = df_removido["Atacado"].map(
            lambda x: f"R$ {x:,.2f}".replace(",", "X")
            .replace(".", ",")
            .replace("X", ".")
        )
        if habilita_preco_cx:
            df_removido["R$ Venda"] = df_removido["Venda"].map(
                lambda x: f"R$ {x:,.2f}".replace(",", "X")
                .replace(".", ",")
                .replace("X", ".")
            )
        ...
        if not df_removido.empty and len(df_removido) == len(df_completo):
            st.write(
                f":material/List: Total Itens na lista: :red[{len(df_removido)} itens]"
            )
            ...
            st.write(
                f":material/List: Total Itens na lista: :blue[{len(df_removido)} itens]"
            )
    ...
    if habilita_preco_cx:
        st.dataframe(
            df_removido[
                ["CodProduto", "Descricao", "R$/Un", "R$ Venda", "Fornecedor", "Estoq"]
        ...
        st.dataframe(
            df_removido[
                ["CodProduto", "Descricao", "Lojas", "Atacado", "Fornecedor", "Estoq"]
            ].sort_values("Fornecedor"),
            hide_index=True,
        )
    ...
    if st.toggle("Mostrar Preços com Atenção"):
        ...
        ].sort_values(by="CodProduto")
        df_atencao["Dif_Preco"] = df_atencao["Lojas"] - df_atencao["Atacado"]

        st.markdown("### Mesmo Preço - Lojas e Atacado")
        df_atencao_view = df_atencao[df_atencao["Dif_Preco"] == 0].sort_values(
            by="Estoq", ascending=False
        )
        df_atencao_view
        ...
        df_atencao_view = df_atencao[df_atencao["Dif_Preco"] > 0].sort_values(
            by="Dif_Perc", ascending=False
        )
        df_atencao_view

        st.markdown("### Mais que 50% para Atacado")
        df_atencao_view = df_atencao[df_atencao["Dif_Perc"] <= -20].sort_values(
            by="Dif_Perc"
        )
        df_atencao_view
    ...
    st.info(
        "Insira _'00001produto.txt'_ e _'00001produtoextra.txt'_ e _'00001produtotabela.txt'_ para começar a edição"
    )
